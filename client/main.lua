...
local function isAuthorized(door)
	...
	if door.pincode and door.pincode ~= false then
		return true
	end
...
RegisterNetEvent('qb-doorlock:client:addNewDoor', function()
	...
	local dialog = exports['qb-input']:ShowInput({
		...
		inputs = {
			...
			},
			{
				...
				type = "number",
				isRequired = false,
				default = Config.SaveDoorDialog and doorData.pincode,
			},
			{
	...
	if doorData.pincode == '' then doorData.pincode = false end
...
RegisterCommand('toggledoorlock', function()
	...
	if closestDoor.data.pincode ~= false and closestDoor.data.pincode then
		if not closestDoor.data.locked then TriggerServerEvent('qb-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) return end
		local input = lib.inputDialog('Enter Pin', {
			{type = 'number', label = 'Number input', icon = 'calculator'},
		})
		if not input then return end
		if input[1] ~= tonumber(closestDoor.data.pincode) then
			QBCore.Functions.Notify("Incorrect Pincode", "error", 2500)
		elseif input[1] == tonumber(closestDoor.data.pincode) then
					...
					elseif paused then
						...
						if closestDoor.distance < (closestDoor.data.distance or closestDoor.data.maxDistance) then
							...
							if not closestDoor.data.hideLabel and Config.UseDoorLabelText and closestDoor.data.doorLabel then
								...
								if closestDoor.data.pincode and closestDoor.data.locked then
									displayText = "[E] - Enter Pin"
								elseif not closestDoor.data.locked and not authorized then
